...
public class TestProperties {
	...
	public BigDecimal getBigDecimalProperty(final String key, final double defaultValue, int scale) {
		BigDecimal bigDecimalValue = getBigDecimalValue(defaultValue, scale);
		final String value = this.props.getProperty(key);
		if (value != null) {
			bigDecimalValue = getBigDecimalValue(Double.parseDouble(value), scale);
		}
		return bigDecimalValue;
	}

	public BigDecimal getBigDecimalProperty(final String key, final double defaultValue) {
		return getBigDecimalProperty(key, defaultValue, 2);
	}
